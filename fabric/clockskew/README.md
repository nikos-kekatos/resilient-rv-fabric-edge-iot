...
The paper's skew experiment injects offsets onto *recorded* timestamps. These two
...
## Option 2 — two VMs (strongest, real separate kernels + network)
`two_vm_skew.sh` boots two Lima VMs, disables NTP on the second and steps its clock by
`SKEW_S` seconds, runs one gateway per VM against a broker on the host, and measures
the realised `eps`. This is the faithful setup for the headline P3.6/P3.7 claim.
```
brew install lima
...
disciplining. This replaces "synthetic offsets" with a measured resilience bound.
